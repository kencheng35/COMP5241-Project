...
export default async function Lesson({ params }: { params: Promise<{ slug: string; lesson: string }> }) {
  ...
  return <LearnerShell active="/dashboard" name={await displayName(user)} admin={isAdmin(user)}><div className="portal-page"><Link className="back-link" href={`/courses/${lesson.id}`}>Lesson overview</Link><h1>{lesson.title}</h1><LessonPlayer id={lesson.id} version={lesson.version} content={content} saved={restored?.success ? restored.data : null} revision={saved?.revision ?? 0} resumeAvailable={!resumeError} /><LessonCoach id={lesson.id} /></div></LearnerShell>;
}