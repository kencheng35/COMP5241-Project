...
type ProfileValues = { displayName: string; email: string; ageRange: string; level: string; subjects: string; goals: string; avatar: string | null; initials: string };
...
export function ProfileForm({ initial, notice }: { initial: ProfileValues; notice: { message: string; isError: boolean } | null }) {
  ...
  return <form ref={formRef} action={action} onSubmit={() => setSubmitted(true)} className="settings-panel">
    ...
    <div className="settings-fields">
      ...
      <label>Age range<select key={state.fields?.ageRange ?? "initial"} name="ageRange" defaultValue={fields.ageRange}><option value="under-13">Under 13</option><option value="13-17">13–17</option><option value="18-24">18–24</option><option value="25-34">25–34</option><option value="35-plus">35 or above</option></select></label>
